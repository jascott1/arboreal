resource "null_resource" "ansible-provision" {
  ...
  provisioner "local-exec" {
    command =  "echo \"[kube-masters]\n${openstack_compute_instance_v2.k8s-master.access_ip_v4} ansible_ssh_host=${openstack_compute_instance_v2.k8s-master.access_ip_v4}\" > k8s-inventory"
  }
  ##Create ETCD Inventory
//  provisioner "local-exec" {
//    command =  "echo \"\n[etcd]\n${openstack_compute_instance_v2.k8s-master.name} ansible_ssh_host=${openstack_compute_instance_v2.k8s-master.access_ip_v4}\" >> k8s-inventory"
//  }
  ...
  provisioner "local-exec" {
    command =  "echo \"\n[kube-control]\n${openstack_compute_instance_v2.k8s-master.name}\" >> k8s-inventory"
  }
